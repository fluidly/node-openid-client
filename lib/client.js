...
<body onload="javascript:document.forms[0].submit()">
  ...
  authorizationCallback(redirectUri, parameters, checks = {}) {
    ...
    if (params.id_token) {
      jsonLogger.info({ message: 'SSO OpenId Client authorizationCallback', id_token: params.id_token });
  ...
  grant(body) {
    ...
    return this.authenticatedPost('token', { body: _.omitBy(body, _.isUndefined) })
      ...
      .then((response) => {
        ...
        jsonLogger.info({ message: 'SSO OpenId Client grant', response: response.body });
        return new TokenSet(JSON.parse(response.body));
      })
      .catch((err) => {
        jsonLogger.info({ message: 'SSO OpenId Client error', err });
        throw err;
      });
  }
