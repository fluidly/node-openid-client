...
<body onload="javascript:document.forms[0].submit()">
  ...
  endSessionUrl(params = {}) {
    ...
    if (hint instanceof TokenSet) {
      assert(hint.id_token, 'endSessionUrl() id_token not present in TokenSet');
      hint = hint.id_token;
    }
   ...
   * @api public
   */
  authorizationCallback(redirectUri, parameters, checks = {}, logger) {
    ...
    if (params.id_token) {
      logger({ message: 'SSO OpenId Client authorizationCallback', id_token: params.id_token });
      promise = Promise.resolve(new TokenSet(params))
        .then((tokenset) => {
          logger({ message: 'SSO begin decryptIdToken' });
          return this.decryptIdToken(tokenset);
        })
        .then((tokenset) => {
          logger({ message: 'SSO begin validateIdToken' });
  ...
  decryptIdToken(token, use) {
    ...
    if (idToken instanceof TokenSet) {
      assert(idToken.id_token, 'decryptIdToken() id_token not present in TokenSet');
  ...
  validateIdToken(tokenSet, nonce, returnedBy, maxAge, state) {
    ...
    if (isTokenSet) {
      assert(idToken.id_token, 'validateIdToken() id_token not present in TokenSet');
      idToken = idToken.id_token;
    }
   ...
   */
  grant(body, logger = () => {}) {
    console.log({ message: 'SSO OpenId Client begin grant' });
    logger({ message: 'SSO OpenId Client begin grant' });
    assertIssuerConfiguration(this.issuer, 'token_endpoint');
    return this.authenticatedPost('token', { body: _.omitBy(body, _.isUndefined) })
      .then(expectResponseWithBody(200))
      .then((response) => {
        console.log({ message: 'SSO OpenId Client grant', response: response.body });
        logger({ message: 'SSO OpenId Client grant', response: response.body });
        return new TokenSet(JSON.parse(response.body));
      })
      .catch((err) => {
        logger({ message: 'SSO OpenId Client error', err });
        throw err;
      });
  }
